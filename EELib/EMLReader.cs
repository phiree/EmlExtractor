            ...
            foreach (string sHdr in saLines)
            {
                ...
                switch (saHdr[0].ToLower())
                {
                    ...
                    case "cc":
                        ...
                        break;
                    case "date":
                        string dateWithRightFormat = saHdr[1];
                        int tempindex=dateWithRightFormat.IndexOfAny(new char[]{'-','+'});
                        if(tempindex>=0)
                        {
                            dateWithRightFormat = dateWithRightFormat.Substring(0, tempindex);
                        }
                        /*Tue, 19 Mar 2013 06:49:51 +0800 (CST)"*/
                       // string dateWithRightFormat=dateTempS.Substring(0,
                        Date = DateTime.Parse(dateWithRightFormat);
                        break;
                    case "subject":
                        Subject = saHdr[1];
                        break;
